...
class ChengeUserData
{
	...
	public function get_send_mail_status($user) {
		try {
			$conn = new PDO("mysql:host=$this->db_location;dbname=$this->db_name", $this->db_user, $this->db_password);
			$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

			$get_from_table = $conn->prepare($this->get_comment_mail_value($user));
			$get_from_table->execute();
			$is_value = $get_from_table->fetchAll();
			return ($is_value[0][0]);
	...
	private function get_comment_mail_value($user) {
		return ("SELECT comment_mail FROM `users` WHERE `login`='$user'");
	}

	public function set_send_mail_status($value, $user) {
		try {
			$conn = new PDO("mysql:host=$this->db_location;dbname=$this->db_name", $this->db_user, $this->db_password);
			$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			if ($value == 'true')
				...
				$value = 'true';
			$conn->exec($this->set_send_mail_table($value, $user));
		} catch (PDOException $e) {
			echo "Connection failed: " . $e->getMessage();
		}
	}

	private function set_send_mail_table($value, $user) {
		return ("UPDATE `users` SET comment_mail = '$value' WHERE `login`='$user'");
	}
}
?>
