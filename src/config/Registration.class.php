...
class Registrations
{
    ...
    public function __construct($username, $user_email, $user_pass, $user_pass_rep)
      ...
      $message = 'Thanks for signing up!
                        ...
                        http://localhost:8100/config/verify.php?email='.$this->user_email.'&hash='.$this->verifications_code.'';
    ...
    public function send_user_pass_onmail($user_mail, $db_location, $db_name, $db_user, $db_password) {
      try {
        $conn = new PDO("mysql:host=$db_location;dbname=$db_name", $db_user, $db_password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				$find_user = $conn->prepare("SELECT `login` FROM `users` WHERE `email`='$user_mail'");
				$find_user->execute();
				$is_user = $find_user->fetchAll();
				if (isset($is_user[0][0])) {
					/*generet new pass*/
					...
					$user_name = $is_user[0][0];
					$pass_for_table = hash('sha256', implode($pass));
					$pass_for_user = implode($pass);
					$subject = 'Forgot password camaro';
					$message = 'We strongly recommend to change your password in your profile on website!
											Yor new password : '.$pass_for_user.'';
					$header = 'MIME-version: 1.0' . "\r\n";
					$header .= 'Content-Type:text/html;charset=UTF-8' . "\r\n";
					$header .= 'From: noreply@camagru.com' . "\r\n";
					if (mail($user_mail, $subject, $message, $header)) {
						$conn->exec("update `users` set `password`='$pass_for_table' where `login`='$user_name'");
						echo 'change';
					}
				}
